...
This module provides functions to fit a multivariate causal forest. Most importantly it
...
from mcf.multivariate_tree import fit_causaltree
from mcf.multivariate_tree import predict_causaltree
...
class CausalForest:
    ...
    def fit(self, X, t, y1, y2):
        ...
        """
        _assert_data_input_cforest(X, t, y1, y2)
        XX, tt, yy1, yy2 = np.array(X), np.array(t), np.array(y1), np.array(y2)

        fitted_model, N_counts = fit_causalforest(
            X=XX,
            t=tt,
            y1=yy1,
            y2=yy2,
    ...
    def predict(self, X, num_workers=None):
        ...
        predictions = predict_causalforest(
            self.fitted_model, self.N_counts, XX, num_workers=num_workers
        )
        return predictions
...
def fit_causalforest(
    X, t, y1, y2, forestparams, treeparams, num_workers, seed_counter
):
    ...
    *num_workers* in dictionary *forestparams* is set to an integer greater
    ...
    Args:
        ...
        y1, y2 (np.array): 1d arrays containing outcomes.
    ...
    Returns:
        cforest (pd.DataFrame): fitted causal forest represented in a pandas
            data frame.
         N_counts (np.array): number of times i-th observation occurs in a b-th bootstrapped sample  
    ...
    N_counts = [tmp[2] for tmp in parallel_result]


    cforest = _construct_forest_df_from_trees(ctrees, features)
    return cforest, N_counts


def _fit_single_tree_for_forest(
    i, X, t, y1, y2, treeparams, n, p, seed_sequence, split_ratio
):
    ...
    ctree = fit_causaltree(
        ...
        t=t[resample_index],
        y1=y1[resample_index],
        y2=y2[resample_index],
        critparams=treeparams,
    )
    
    # count how many times obs. i appears in a b-th bootstrap sample
    values = np.array(list(Counter(resample_index).values()))  ## count indices in resample_index
    keys = np.array(list(Counter(resample_index).keys()))
    data_obs = np.arange(0, n)
    diff = list(set(data_obs) - set(keys)) ## find the data that did not occur in a b-th sample and add that many 0s.
    zero_array = np.zeros(len(diff)) 
    N_counts = np.append(values, zero_array) # this addition is needed for jackknife variance
    #print(len(ctree))
    out = [ctree, feature_index, N_counts]
    return out
...
def predict_causalforest(cforest, N_counts, X, num_workers):
    ...
    *num_workers* processes.
    ...
    Returns:
        predictions1 and predictions2 (np.array): 1d arrays of treatment predictions.
        std_y1 and std_y2 (np.array): 1d arrayss of jackknife standard deviation.
    ...
    predictions = [arr.reshape((2, n)) for arr in predictions]
    
    
    predictions1_raw = [arr[0, :] for arr in predictions]
    predictions1 = np.vstack(predictions1_raw)
    predictions1 = np.round(predictions1.mean(axis = 0), 3) # 0 is below, 1 is mean across --->

    predictions2_raw = [arr[1, :] for arr in predictions]
    predictions2 = np.vstack(predictions2_raw)
    predictions2 = np.round(predictions2.mean(axis = 0), 3)
    
    N_counts_array = np.array(np.vstack(N_counts))

    ## Jackknife variance for the lower bound (predictions1)
    predictions_raw_new_1 = np.array([arr - predictions1 for arr in predictions1_raw])
    predictions_raw_new_1 = np.array(np.vstack(predictions_raw_new_1))
    jack = (np.dot((N_counts_array - 1).T,  predictions_raw_new_1)/num_trees)**2
    jack = jack.sum(axis = 0)
    bias_correction_1 = ((n)/num_trees**2)*((predictions_raw_new_1**2).sum(axis = 0)) # 0 is below, 1 is sum across --->

    jackknife_1 = jack  - bias_correction_1
    
    
    # Jackknife variance for the upper bound (predictions2)
    predictions_raw_new_2 = np.array([arr - predictions2 for arr in predictions2_raw])
    bias_correction_2 = ((n)/num_trees**2)*((predictions_raw_new_2**2).sum(axis = 0)) # 0 is below, 1 is sum across --->
    predictions_raw_new_2 = np.array(np.vstack(predictions_raw_new_2))
    jack = (np.dot((N_counts_array - 1).T,  predictions_raw_new_2)/num_trees)**2
    jack = jack.sum(axis = 0)
    jackknife_2 =  jack  - bias_correction_2
    
    std_y1 = np.sqrt(jackknife_1)
    std_y2 = np.sqrt(jackknife_2)

    predictions2 = predictions2.tolist()


    return predictions1, predictions2, std_y1, std_y2


def _assert_data_input_cforest(X, t, y1, y2):
    ...
    if isinstance(y1, np.ndarray):
        if np.isnan(np.sum(y1)):
            raise ValueError("Data on outcomes *y1* contains NaNs.")
        ny = len(y1)
    elif isinstance(y1, pd.Series):
        if y1.isnull().any():
            raise ValueError("Data on outcomes *y1* contains NaNs.")
        ny = len(y1)
    else:
        raise TypeError("Data on outcomes *y1* is not a pd.Series or np.array.")
    ...
    if isinstance(y2, np.ndarray):
         if np.isnan(np.sum(y2)):
             raise ValueError("Data on outcomes *y2* contains NaNs.")
         ny = len(y2)
         ...
         raise TypeError("Data on outcomes *y2* is not a pd.Series or np.array.")
